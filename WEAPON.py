...
class MeleeWeapon(Weapon):
	...
	def spawn_hit(self, direction, groups, npcs):
		...
		match direction:
			...
			case 'left':
				return MeleeHit((self._player.rect.centerx - size, self._player.rect.centery), 
				groups, -180, self._player, npcs)

			case 'right':
				return MeleeHit((self._player.rect.centerx + size, self._player.rect.centery), 
				groups, 0, self._player, npcs)

			case 'top':
				return MeleeHit((self._player.rect.centerx, self._player.rect.centery - size), 
				groups, 90, self._player, npcs)
...
class Fists(MeleeWeapon):
	def __init__(self, player):
		super().__init__(
			damage=5, width=40, height=30,
			player=player, filename='')

class Bat(MeleeWeapon):
	def __init__(self,player):
		super().__init__(damage=8, 
		width=130, height=40, player=player, 
		filename='materials/weapon/melee/bat/bat.png')
